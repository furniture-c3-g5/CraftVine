...
const getProfile = async (req, res) => {
  ...
  try {
    const userQuery = "SELECT user_id, email, first_name, last_name, phone, profile_image_name FROM users WHERE user_id = $1";
    ...
    if (userResult.rows.length > 0) {
      ...
      const images = userResult.rows.map((image) => {
        ...
        const image_url = profile_image_name ? `http://localhost:5000/ProductImages/${profile_image_name}` : null;
        return { 
          profile_image_name: image.profile_image_name, 
          image_url: image_url 
        };
      });
      res.status(200).json({ message: "Get Profile Successfully", data: user, images: images });
    } else {
