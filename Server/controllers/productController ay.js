...
const getBroductById = async (req, res) => {
  ...
  try {
    const productQuery =
      "SELECT p.product_id, p.product_name, p.category, p.price, p.description, p.quantity, p.release_date, pi.image_id, pi.image_name, pi.product_color " +
      "FROM products p " +
      "LEFT JOIN product_images pi ON p.product_id = pi.product_id " +
...
const addNewProduct = async (req, res) => {
  ...
  try {
    const productInsertQuery =
      "INSERT INTO products (product_name, category, price, description, quantity, release_date) VALUES ($1, $2, $3, $4, $5, $6) RETURNING product_id";
    ...
    if (productResult.rows.length > 0) {
      ...
      for (let i = 0; i < images.length; i++) {
        ...
        const productImagesInsertQuery =
          "INSERT INTO product_images (product_id, image_name, product_color) VALUES ($1, $2, $3)";
        await pool.query(productImagesInsertQuery, [
          product_id,
          image_name,
          color,
        ]);
